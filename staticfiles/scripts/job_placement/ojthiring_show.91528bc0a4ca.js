window.onload = function () {
        ...
        success: function (data) {
          ...
          for (let i = 0; i < data.length; i++) {
            ...
            $("#search_suggestions").append(
              '<li onclick="getstudent(this)" value="' +
                data[i][0] +
                '">' +
                text +
                "</li>"
    ...
    if (query.length > 0) {
      $.ajax({
        ...
        success: function (data) {
          ...
          for (let i = 0; i < data.length; i++) {
            text = `${data[i][0]} - ${data[i][1]}, ${data[i][2]}`;
            $("#assign_student_suggestions").append(
              '<li onclick="assign_stud_id(this)" value="' +
                data[i][0] +
                '">' +
                text +
                "</li>"
  ...
  $("#assign_company_id").on("input", function () {
    ...
    if (query.length > 0) {
      $.ajax({
        url: "/jobplacement/suggestions/companies",
        data: { query: query },
        success: function (data) {
          console.log(data);
          ...
          for (let i = 0; i < data.length; i++) {
            ...
            $("#company_suggestions").append(
              '<li onclick="assign_company_id(this)" value="' +
                data[i][0] +
                '">' +
                text +
                "</li>"
