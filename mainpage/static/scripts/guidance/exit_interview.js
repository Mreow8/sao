$(document).ready(function () {
  ...
  $("#searchButton").on("click", function (event) {
    ...
    $.ajax({
      ...
      error: function (error) {
        ...
        $("#info_table").empty();
        $("#info_table").append(
          "<tr>" + '<td colspan="2">Student ID not found.</td>' + "</tr>"
        );
      },
    });
  });
  $("#id_scheduled_date").on("change", function () {
    if ($(this).val()) {
      ...
      // Make a POST request
      $.post({
        url: "/main/check_date_time_validity_for_exit/",
        ...
        success: function (response) {
          ...
          $.each(disabledTimes, function (index, value) {
            if (value.status != "Declined") {
              $(
                '#id_scheduled_time option[value="' +
  ...
  function validateInputs() {
    ...
    $('table.table input[type="number"]').each(function () {
      ...
      if ($(this).attr("name") === "what_is_your_intended_major") {
        let intendedMajor = $(
          "input[name='what_is_your_intended_major']"
        ).val();
        if (intendedMajor == "") {
      ...
      if (value === "") {
        ...
        $(this).addClass("error");
        $(this).after(
          '<span class="error-message" style="color:red;">Value must be 1 or higher.</span>'
        );
      }
    ...
    if (hasDuplicates) {
      $('table.table input[type="number"]').each(function () {
        ...
        if (value !== "" && values.filter((v) => v === value).length > 1) {
          $(this).addClass("error");
          $(this).after(
            '<span class="error-message" style="color:red;">Duplicate value found.</span>'
          );
        }
      });
    }
