...
      $("#employedYesNo").text(
        "-Please provide the name and address of the company you want:"
      );
    }
  });
...
    $.ajax({
      url: "/main/search_exit_interview_request/",
      method: "POST",
...
    $.post({
      url: "/main/update_exit_interview_status/",
...
    $.post({
      url: "/main/get_exit_interview_request/",
...
        // ... (populating all table_data elements) ...
...
        let recommed = response.recommed ? "Yes" : "No";
...
  // --- Save Button Click (Download PDF) ---
  $(document).on("click", ".saveButton", function () {
    const elements = document.getElementById("paper");
    const student_name = $("#student-name").text() || "student"; // Fallback name
    const options = {
      margin: [0, 0, 0, 0],
...
    $.post({
      url: "/main/delete_exit_interview_status/",
...
    if (!el) {
      console.error("Print: #paper element not found");
      return;
    }
    const opt = {
      margin: 0.5,
...
    // Generate blob, open in new window and trigger print
    html2pdf()
      .set(opt)
      .from(el)
      .outputPdf("blob")
...
        // Write an iframe with the blob and call print
        w.document.write(
...
        // Attempt to print once content loads
        const tryPrint = function () {
          try {
            w.focus();
            w.print(); // <-- THIS OPENS THE PRINT DIALOG
          } catch (err) {
