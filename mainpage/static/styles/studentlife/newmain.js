...
// Safe calendar initialization: do not crash the rest of the scripts
(function initCalendarSafely() {
  try {
    var calOptions = {
      selectable: true,
      // ...existing calendar options you have...
    };

    // legacy jQuery FullCalendar (v1-3)
    if (
      typeof $.fn.fullCalendar === "function" &&
      document.getElementById("calendar")
    ) {
      calOptions.selectHelper = true; // only set when plugin exists
      $("#calendar").fullCalendar(calOptions);
    }
    // FullCalendar v4+ (class API)
    else if (
      typeof FullCalendar !== "undefined" &&
      document.getElementById("calendar")
    ) {
      calOptions.selectMirror = true;
      ...
      console.debug(
        "No supported calendar library found; skipping calendar init."
      );
    }
  } catch (err) {
    console.error("Calendar init error (non-fatal):", err);
  }
})();
