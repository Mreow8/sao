...
def patient_basic_info(request, student_id):
    ...
    if Patient.objects.filter(student__student_id = student_id).exists():
        ...
        return render(request, "students/basicinfo.html", {"student": student, "patient": patient, "user_email": request.user.email})
    return render(request, "students/basicinfo.html", {"student": student, "user_email": request.user.email})

# view for handling clearance form submission
@medical_admin_required
    ...
    if request.method == "POST":
        ...
        try:
            if action == "approve":
                ...
                patient_email = patient_request.patient.user_email
                ...
                # Create an email backend connection with the specific email settings
                email_backend = get_connection(
                    backend='django.core.mail.backends.smtp.EmailBackend',
                    host="smtp.gmail.com",
                    port=587,
                    username="ctuargaoclinic@gmail.com",
                    password="ttaokfbrkovxfhjn",
                    use_tls=True
                )
                ...
                send_mail(
                    ...
                    '',
                    "ctuargaoclinic@gmail.com",  # From email
                    [patient_email],
    ...
    if request.method == "POST":
        ...
        <body style='font-family: Arial, sans-serif; line-height: 1.6; color: #333;'>
            <div style='max-width: 600px; margin: auto; padding: 20px; border: 1px solid #ccc; border-radius: 10px; background-color: #f9f9f9;'>
                ...
                <p>If you have any questions or need further assistance, feel free to reply to this email or visit us in the Kahimsug Center.</p>
                <p>Best Regards,</p>
...
def upload_requirements(request):
    ...
    if request.method == "POST":
        ...
        if (x_ray and (x_ray.size / 1_048_576) > 5) or (cbc and (cbc.size / 1_048_576) > 5) or (stool_exam and (stool_exam.size / 1_048_576) > 5) or (drug_test and (drug_test.size / 1_048_576) > 5):
            ...
            return render(request, "students/medupload.html", {"patient": patient, "student_id": student_id})
        ...
        try:
            ...
            if pwd_card:
                file_name = pwd_card.name
                _, file_extension = os.path.splitext(file_name)
                if file_extension.lower() not in ['.jpg', '.png', '.jpeg']:
                    messages.info(request, "Make sure the pwd card you uploaded is an image.")
            ...
            if pwd_card:
                ...
                if file_extension.lower() not in ['.jpg', '.png', '.jpeg']:
                    messages.info(request, "Make sure the pwd card you uploaded is an image.")
                    return render(request, "students/medupload.html", {"student_id": student_id})
                md.pwd_card.save(pwd_card.name, pwd_card)

            messages.success(request, "Your files have been successfully uploaded")

        return render(request, "students/medupload.html", {"patient": patient, "md": md, "student_id": student_id})
...
def dental_services(request):
    ...
    if request.method == "POST":
        ...
        if not valid_id:
            messages.error(request, "The ID you entered does not exist.")
            return render(request, "students/dentalrequestform.html", {"student_id": student_id})
...
def dental_request(request):
    ...
    if request.method == "POST":
        try:
            ...
            patient_name = f"{patient.student.firstname} {patient.student.lastname}"
            patient_email = patient_dental_request.patient.user_email
            ...
            <body style='font-family: Arial, sans-serif; line-height: 1.6; color: #333;'>
                <div style='max-width: 600px; margin: auto; padding: 20px; border: 1px solid #ccc; border-radius: 10px; background-color: #f9f9f9;'>
                    ...
                    <p>Dear <strong>{patient_name}</strong>,</p>
                    <p>We're pleased to inform you that your appointment date has been set for <strong>{appointment_datetime.strftime("%B %d, %Y")}</strong> at <strong>{appointment_datetime.strftime("%I:%M %p")}</strong>.</p>
...
def transactions_view(request):
    transaction_type = request.GET.get('type', 'All Transactions')
...
def monthly_transactions_view(request):
    if request.method == 'POST':
        ...
        transaction_type = request.POST.get('type', 'All Transactions')
